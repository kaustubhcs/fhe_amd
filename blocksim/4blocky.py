...
list_of_kernels = ['ntt_32', 'intt_32', 'point_mult_32', 'scalar_mult_32', 'point_add_32',
                   ...
                   'scalar_mult_64', 'point_add_64', 'point_sub_64', 'rotate_64', 'big_he_mult_64', 'big_he_rescale_64', 'big_he_rotate_64']
...
if USE_32_BIT:
    ...
    k_rotate = 'rotate_64'
    big_k_he_mult = 'big_he_mult_64'
    big_k_he_rescale = 'big_he_rescale_64'
    big_k_he_rotate = 'big_he_rotate_64'
...
def tas(uid, compute_graph):
	start_node = uid
	compute_graph, uid = add_node_to_graph(compute_graph, k_null, uid, block_data, 'tas')
	end_node = uid
	compute_graph, uid = add_node_to_graph(compute_graph, k_null, uid, block_data, 'tas')
	...
	for i in range(he_mult_count):
		uid, compute_graph, start_node_0, end_node_0 = he_mult(uid, compute_graph)
		compute_graph.add_edge(start_node, start_node_0)
		compute_graph.add_edge(end_node_0, intermediate_node_0)

	# Bootstrapping
	uid, compute_graph, start_node_1, end_node_1 = bootstrapping(uid, compute_graph)
	compute_graph.add_edge(intermediate_node_0, start_node_1)
	compute_graph.add_edge(end_node_1, end_node)
	return uid, compute_graph, start_node, end_node
...
def logistic_regression(uid, compute_graph):
	...
	for i in range(he_mult_count):
		he_mult_node = uid
		compute_graph, uid = add_node_to_graph(compute_graph, big_k_he_mult, uid, block_data, 'logistic_regression')
		# uid, compute_graph, start_node_1, end_node_1 = he_mult(uid, compute_graph)
		compute_graph.add_edge(intermediate_node_0, he_mult_node)
		compute_graph.add_edge(he_mult_node, intermediate_node_1)
	...
	for i in range(rescale_count):
		he_rescale_node = uid
		compute_graph, uid = add_node_to_graph(compute_graph, big_k_he_rescale, uid, block_data, 'logistic_regression')
		# uid, compute_graph, start_node_2, end_node_2 = he_rescale(uid, compute_graph)
		compute_graph.add_edge(intermediate_node_1, he_rescale_node)
		compute_graph.add_edge(he_rescale_node, intermediate_node_2)
	...
	for i in range(rotate_count):
		he_rotate_node = uid
		compute_graph, uid = add_node_to_graph(compute_graph, big_k_he_rotate, uid, block_data, 'logistic_regression')
		# uid, compute_graph, start_node_3, end_node_3 = he_rotate(uid, compute_graph)
		compute_graph.add_edge(intermediate_node_2, he_rotate_node)
		compute_graph.add_edge(he_rotate_node, intermediate_node_3)
...
def resnet20(uid, compute_graph):
	...
	for i in range(he_mult_count):
		he_mult_node = uid
		compute_graph, uid = add_node_to_graph(compute_graph, big_k_he_mult, uid, block_data, 'resnet20')
		# uid, compute_graph, start_node_1, end_node_1 = he_mult(uid, compute_graph)
		compute_graph.add_edge(intermediate_node_0, he_mult_node)
		compute_graph.add_edge(he_mult_node, intermediate_node_1)
	...
	for i in range(rescale_count):
		he_rescale_node = uid
		compute_graph, uid = add_node_to_graph(compute_graph, big_k_he_rescale, uid, block_data, 'resnet20')
		# uid, compute_graph, start_node_2, end_node_2 = he_rescale(uid, compute_graph)
		compute_graph.add_edge(intermediate_node_1, he_rescale_node)
		compute_graph.add_edge(he_rescale_node, intermediate_node_2)
	...
	for i in range(rotate_count):
		he_rotate_node = uid
		compute_graph, uid = add_node_to_graph(compute_graph, big_k_he_rotate, uid, block_data, 'resnet20')
		# uid, compute_graph, start_node_3, end_node_3 = he_rotate(uid, compute_graph)
		compute_graph.add_edge(intermediate_node_2, he_rotate_node)
		compute_graph.add_edge(he_rotate_node, intermediate_node_3)
...
def test_tas():
	...
	graph_end_node = uid
	compute_graph, uid = add_node_to_graph(compute_graph, k_null, uid, block_data, 'init')
	uid, compute_graph, start_node, end_node = tas(uid, compute_graph)
	compute_graph.add_edge(graph_start_node, start_node)
	compute_graph.add_edge(end_node, graph_end_node)
	graph_validation(compute_graph)
	return compute_graph
...
def gpu_ready_for(node, computing_list, compute_graph, block_data):
	...
	for computing_node in computing_list:
		...
		if node_type == 'null':
			...
			navisim_dram_bw = block_data[node_type]['DRAM Bandwidth']
			################################### DRAM BYPASS ###################################
			if navisim_dram_bw == 0:
				iter_dram_bw = 0
			################################### DRAM BYPASS ###################################
